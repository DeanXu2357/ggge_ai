...
## 路由與 middleware（2026-07-18 落地）

`run()` 每輪先過**中斷路由**（`_route_interrupts`：terminal／defeat／hidden／
known_modal／story／end_turn 依優先序＋成本排的 detect→handle 管線），沒有
中斷認領才落到**相位路由**（`_dispatch_phase` 依 mode 派到 `_on_*`）。兩層都
是「機制」，log 內容仍由 handler 依畫面決定，不違反紅線。

log 走 middleware，handler 不直接碰 ledger：

- **中斷路由**：handler 回傳 `LoopStep`，用 `log=(kind, extras)` 宣告要記什麼、
...
- **相位路由**：`_dispatch_phase` 只記**信封**——handler 正常結束記一筆
  ...
  `LoopStep(done=True)` 讓主迴圈跟中斷走同一個離場接縫。
