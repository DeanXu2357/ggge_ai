...
判斷依據（**2026-07-09 落地時修正**，見下方說明）：`_current_mode()
is not None`——`MODE_LABELS` 五個模板（`our_turn`／`unit_move`／
`weapon_select`／`skill`／`battle_prep`）任一命中。
...
`vision.unit_cards_present` 沒有被拿掉，還是用在原本的地方——
`_on_our_turn()` 內部判斷「列表裡還有沒有可操作單位、該選下一個還是
結束回合」，這是它已經被全語料驗證過的用途。

**跟原始設計的落差**：本文最初把 `unit_cards_present` 設計成頂層
ACTIONABLE 的判斷依據（下面 NOT_ACTIONABLE 的判斷依據段落原文也是這樣
寫的），但落地寫程式碼前發現一個問題：**沒有任何實機證據能確認卡片列
在 `UNIT_MOVE`／`WEAPON_SELECT` 這些子畫面底下是不是還留著可見**——
現有語料庫裡唯一走到過 select_unit 之後的場次不是卡在同一張畫面
（20260706-233158 的 mystery A stall），就是幀是 0-byte（凍機事故
截斷）。貿然假設卡片列全程可見，等於是在賭一個沒有證據的假設，違反
CLAUDE.md「沒有新截圖證據不要動閾值/假設」的紀律。改用 `mode is not
None` 完全不用賭：這就是現有 handler 已經在用、已經跑得動的訊號，
這次重構純粹是把「`mode is None` 時做什麼」從匿名 fallback 變成一個
有名字的方法（`_on_not_actionable`），行為完全不變（167 測試全過，
含專門釘住這個等價性的回歸測試）。真的要驗證 `unit_cards_present`
能不能撐起更大範圍的判斷，需要裝置。
...
模板，跟其他四個 `MODE_LABELS` 同層級，落地後自然歸在 ACTIONABLE
的第一個子狀態，不需要額外的特例判斷（原本設想它要獨立於循環之外，
是基於 `unit_cards_present` 版本的舊設計；`mode is not None` 版本
直接涵蓋它）。
