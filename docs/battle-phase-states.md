...
`run()` 每個 tick 只做三件事：抓一張共用幀 → `_classify` 判斷「我在哪」→
`_dispatch` 查表分派，消費回傳的 `LoopStep`（`done`/`screen`＝戰鬥結束、
`activity`＝餵閒置看門狗）。覆蓋層與相位是**同一個狀態列舉的平等成員**，
不再分「中斷層／相位層」兩段路由。

`_classify` 內部依畫面 **z-order 由上往下**讀證據：整頁替換（terminal／
defeat）→ 疊在戰鬥 UI 上的彈窗（hidden_battle／known_modal／story／
end_turn，即 `_overlay_pipeline` 表，掃描順序與 state→handler 配對同一張表）
→ 最後才信基態的相位 label（`_confirmed_mode` 雙讀確認）。順序不可倒的
原因：tap 的語意屬於最上層元素，且模板比對會穿透變暗的覆蓋層（label 在
對話框底下照樣高信心命中，反向不會），所以「覆蓋層全部 decline」是相位
label 可信的前置條件。label 的簿記（state log／expectation 檢查／擊殺
判定）跟著 mode 讀取走，覆蓋層認領的 tick 不會燒 expectation 預算。

log 走 middleware，handler 不直接碰 ledger（`_dispatch` 依 state 選對應的
middleware）：

- **覆蓋層**：handler 回傳 `LoopStep`，用 `log=(kind, extras)` 宣告要記什麼、
  `finish=outcome` 宣告要封存流水帳；`_dispatch_interrupt` 統一補上共用幀並
  執行。好處是格式一致、沒有 handler 能默默漏記——`end_turn` 以前就整段沒進
  ledger，改用宣告式後自動補上。沒宣告 `log` 的 handler（回傳空 `LoopStep()`）
  就是刻意不記，這是「部份 handler 走 middleware」的 opt-in。
- **相位**：`_dispatch_phase` 只記**信封**——handler 正常結束記一筆
  `phase`（`phase`＝去掉 `label_` 的名字、`outcome=ok`、`elapsed_ms`），
  domain log（attack／standby／pilot_step…）仍留在各 `_on_*` handler 內，因為
  那些 payload 只有 handler 執行到一半才存在。兩個終止型例外（`PilotAbort`／
  `SurveyIncomplete`）沿用原本的記錄（`survey_abort`／`_log_finish`），並轉成
  `LoopStep(done=True)` 讓主迴圈跟覆蓋層走同一個離場接縫。
- **NOT_ACTIONABLE**：`_on_not_actionable` 的 bool 回傳包成
  `LoopStep(activity=...)`——純等待與 neutral tap 不算進展（盲戳若重置
  看門狗，認不得的畫面就永遠不會 idle timeout）。
...
生命週期主結構已落地（commit 516987e，後續收攏成 `_classify`／
`_dispatch` 單一接縫，行為等價）。剩下的實質工作只有一項：**標定
ENEMY_REACTION_POPUP 的畫面錨點與 handler**——待裝置。
