...
2. **反擊階段只看主目標存活**：A 活著 → A 應戰＋C 支援攻擊都打 M；
   ...
   A 消音全部回擊」是搜尋樹可利用的戰術（有 solver 測試釘住）。
   **證據狀態注意**：使用者當晚修正案例 2（原記錄「B 有支援防禦但
   預測顯示直接擊殺 A」有誤，實際預測顯示 B 攔截、M 擊殺 B、A 反
   擊），取消規則因此失去原本的實證來源，降級為保守假設（A 自己不
   能反擊是機制必然，C 是否一併停手待實測），樹內仍照此結算。
...
5. 實測案例編成 tests/test_battle_sim.py（case1/case3＝觀測；
   target_kill 取消測試＝假設釘住），另加順序邊界案例（反擊先殺 M
   則 C 停手、無合格攔截者 fall-through 打回 A、擊破攔截者給再動、
   C 出手不看 A 的應戰選項、C 射程不足不出手）。未實測細節以保守
   假設入檔（sim.py docstring＋formulas 文件）。

**新增待實機標定項**：支援防禦觸發規則的實證確認（實測皆「合格即
攔截」但樣本少；對帳鏈 battle_prep 塌陷紀錄持續蒐證）、主目標被擊殺
時 C 是否仍支援攻擊（現假設不會）、支援防禦承受方倍率（現用 0.5
預設）。

**下一步**：沿用稍早快照的實機清單（M3b 驗證、對帳鏈實戰、蛇形掃描
計時）；對帳鏈實戰時順帶記錄「預測畫面顯示的承受者是誰」與「主目標
被擊殺時支援攻擊是否發動」，補上述兩個假設的實證樣本。
