...
- **主動索敵移動**：v1 的移動只依賴「威脅格」（敵方射程覆蓋的格子），敵人離太遠沒有威脅格時
  單位只會原地待機，不會主動朝敵方推進。需要地圖層級的敵人位置感知
  （候選作法：可靠的敵我單位偵測——紅/藍圈弧＋敵我識別修正；或利用「單位列表」面板；
  或移動模式下逐一點擊地圖上的單位讀取其資訊卡判斷敵我），
  然後「無威脅格時朝最近敵人方向移動到最遠可達格」
